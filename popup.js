class NAMNH_CONSOLE {
  ...
  function showPopup2(word){
    var deeplUrl = "https://www.deepl.com/ja/translator#en/ja/" + word;
    namnhlog.e("Link: " + deeplUrl);
    var popupInfo2 = {
        "url": deeplUrl,
        //"type": "normal", // for debug
        "type": "popup",
    ...
    chrome.windows.create(popupInfo2, function(newPopup){
        last_popup_tabId = newPopup.id;
        namnhlog.w("Create last_popup_tabId: " + last_popup_tabId);
    });
  }
...
function myFunction2(){
    document.getElementById("btn_deepl_search").addEventListener("click", function(){
        var txt_deepl_searchfield = document.getElementById("txt_deepl_searchfield").value.trim();
        namnhlog.e("txt_deepl_searchfield: " + txt_deepl_searchfield);
        if(txt_deepl_searchfield)
        {
            // append and save word list
            var storage_listWord_key = "storage_listWord";
            chrome.storage.sync.get([storage_listWord_key], function(result) {
                ...
                array.unshift(txt_deepl_searchfield);
                var jsonObj = {};
                jsonObj[storage_listWord_key] = array;
                chrome.storage.sync.set(jsonObj, function() {
                    namnhlog.e("Saved a new array item: " + array.toString());
            ...
            showPopup2(txt_deepl_searchfield);
            window.parent.document.body.style.zoom=0.30
        }
    });
    // onkey enter
    var txt_deepl_searchfield = document.getElementById("txt_deepl_searchfield");
    txt_deepl_searchfield.addEventListener("keyup", function(event) {
        // Cancel the default action, if needed
        event.preventDefault();
        // Number 13 is the "Enter" key on the keyboard
        if (event.keyCode === 13) {
          // Trigger the button element with a click
          document.getElementById("btn_deepl_search").click();
        }
    });
}
